...
headers = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://x.com/search?q=from%3Alubi366&src=typed_query',
...
def get_latest_tweets():
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] === 开始新一轮检查 ===", flush=True)
    ...
    for username in TARGET_ACCOUNTS:
        try:
            ...
            # === 核心逻辑：动态修改参数 ===
            # 将 "Top" 改为 "Latest"，并将 "lubi366" 替换为当前用户
            # 这样既用了真实指纹，又能抓到不同人的最新推文
                ...
                try:
                    ...
                    if found_tweet:
                        ...
                        if username not in last_seen_ids:
                            ...
                            print(f"🚀 发现新推文! ID: {tweet_id}", flush=True)
                        ...
                        print("未找到有效推文 (可能是空号或 API 结构变化)", flush=True)

                except Exception as parse_error:
                    print(f"解析错误: {parse_error}", flush=True)

            elif response.status_code == 429:
                print("⚠️ 429 请求太多 (Rate Limit)，休息 30 秒...", flush=True)
                time.sleep(30)
            elif response.status_code == 401:
                print("❌ 401 认证失败 (Cookies 过期)", flush=True)
                break # Cookie 失效就停止，防止封号
            else:
                print(f"❌ 失败: {response.status_code}", flush=True)
                # 打印一点点错误详情，方便 Zeabur 排查
                print(f"   详情: {response.text[:200]}", flush=True)

        except Exception as e:
            print(f"❌ 网络/系统异常: {e}", flush=True)
        
        # === 核心修改点：模拟真人阅读，随机等待 8 到 12 秒 ===
        sleep_time = random.uniform(8, 12)
        print(f"   (休息 {sleep_time:.1f} 秒...)", flush=True)
        time.sleep(sleep_time)

    print("=== 本轮结束，等待 16 分钟 ===", flush=True)

# 启动
if __name__ == "__main__":
    print("🔥 [System] 最终完美版监控启动 (间隔加强版)...", flush=True)
