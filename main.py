...
# 1. 【粘贴处】
#    请把刚才搜索 "from:lubi366" 复制到的那条超长链接粘贴在下面！
#    ⚠️ 注意：这串链接里必须包含 "lubi366" 这个词，否则代码会报错。
...
def get_latest_tweets():
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] === [外科手术替换版] 开始检查 ===", flush=True)

    # === 关键检查 ===
    if "lubi366" not in Browser_Link:
        print("❌ 致命错误：Browser_Link 里没找到 'lubi366'！")
        print("请回到浏览器，务必搜索 'from:lubi366'，然后复制那条链接！")
        return
    ...
    for username in TARGET_ACCOUNTS:
        try:
            ...
            # === 核心逻辑：直接字符串替换 ===
            # 不做任何解析，直接把模板里的 lubi366 换成当前要查的人
            # 这样能保留所有乱码、空格编码的一致性
            real_url = Browser_Link.replace("lubi366", username)
            
            # 直接请求 URL，不带 params 参数 (因为都在 URL 里了)
            ...
            if response.status_code == 200:
                ...
                print(" -> ❌ 404 错误！(说明链接模板还是不对，请确认是 Copy URL)", flush=True)
                break 
            elif response.status_code == 401 or response.status_code == 403:
                print(" -> ❌ 认证失败 (请检查 Cookie)", flush=True)
                break
            else:
    ...
    print(f"=== 本轮结束，等待 16 分钟 ===\n", flush=True)

if "graphql" not in Browser_Link:
     print("❌❌❌ 你还没填 Browser_Link！❌❌❌")
else:
