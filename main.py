...
# 【关键修改】精简 Header，移除了 Transaction ID 等容易导致 404 的字段
headers = {
    'accept': '*/*',
    'accept-language': 'en-US,en;q=0.9', # 统一用英文环境，减少指纹冲突
    ...
    # 删除了 x-client-transaction-id
    # 删除了 x-twitter-active-user
    # 删除了 sec-ch-ua 等，因为 curl_cffi 会自动生成这些
}
...
def fetch_data(username, identity):
    """
    使用指定的浏览器身份(identity)发起请求
    """
    try:
        # 动态 Referer，防止 Referer 泄露
        ...
        print(f"   🔥 网络异常 ({identity}): {e}", flush=True)
        return None

def get_latest_tweets():
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] === 开始新一轮 (随机指纹版) ===", flush=True)

    for username in TARGET_ACCOUNTS:
        # 1. 随机抽一张“面具”
        current_identity = random.choice(BROWSER_IDENTITIES)
        print(f"Checking: @{username} [{current_identity}] ... ", end="", flush=True)
        
        response = fetch_data(username, current_identity)

        # 2. 404 重试机制 (换个面具重试)
        if response and response.status_code == 404:
            retry_identity = random.choice(BROWSER_IDENTITIES)
            print(f"⚠️ 404, 换面具({retry_identity})重试... ", end="", flush=True)
            time.sleep(random.uniform(3, 6))
            response = fetch_data(username, retry_identity)
        ...
        # 冷却时间
        sleep_time = random.uniform(10, 16)
...
if __name__ == "__main__":
    print("🔥 [System] 随机指纹去毒版启动...", flush=True)
    get_latest_tweets()
