...
def get_latest_tweets():
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] === 开始新一轮 (全量补全模式) ===", flush=True)
    ...
    for username in TARGET_ACCOUNTS:
                ...
                for entry in entries:
                    if 'tweet' in entry['entryId']:
                        ...
                        if tweet_data:
                            ...
                            # 如果是第一次运行，或者该推文ID 比 记录的ID 大（更新）
                            # 注意：字符串比较 '200' > '199' 在推特ID这种长度一致的情况下是成立的
                            if username not in last_seen_ids:
                                # 第一次运行，只记录最新的一条作为基准，不发送历史消息
                                last_seen_ids[username] = tid
                                print(f"✅ 初始化: {tid}", flush=True)
                                break # 退出内层循环，只拿最新的做锚点
        ...
        # === 间隔控制 ===
        # 10到18秒：比之前稍微快一点，但比最早的8秒安全
        sleep_time = random.uniform(10, 18)
        print(f"   (冷却 {sleep_time:.1f}s)", flush=True)
        time.sleep(sleep_time)

    # === 每轮结束等待时间 ===
    # 改为 12 分钟 (你之前的16分钟有点长，12分钟足够冷却了)
    print("=== 等待 12 分钟 ===", flush=True)

if __name__ == "__main__":
    print("🔥 [System] 多推文补全版启动...", flush=True)
    get_latest_tweets()
